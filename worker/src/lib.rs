...
pub async fn fetch(req: Request, env: Env, _ctx: Context) -> Result<Response> {
    ...
    router
        ...
        .get_async("/api/selectors/:code_type", |_req, ctx| handle_get_selectors(ctx))
        .post_async("/api/selectors/:code_type", |req, ctx| handle_post_selectors(req, ctx))
        .delete_async("/api/selectors/:code_type", |req, ctx| handle_delete_selectors(req, ctx))
        .options("/api/quote", cors_preflight)
        .options("/api/selectors/:code_type", cors_preflight)
        .run(req, env).await
}
...
// 例: テストや管理APIで
#[worker::test]
async fn check_selectors_against_urltest() -> Result<()> {
    let url = "https://finance.yahoo.co.jp/quote/USDJPY=X";
    let result = check_selectors_against_url(url, CodeType::Fx).await?;
    for (selector, is_valid) in result {
        println!("{}: {}", selector, is_valid);
    }
    Ok(())
}
...
// 例: 管理APIや管理画面から新しいセレクターを受け取った場合
#[worker::test]
async fn testauto_update_invalid_selectors(env: Env) -> Result<()> {
let url = "https://finance.yahoo.co.jp/quote/USDJPY=X";
let code_type = CodeType::Fx;
let new_selectors = SelectorConfig {
    name_selector: Some("h2.new-class".to_string()),
    // ...他も必要に応じて
    ..Default::default()
};
let kv = env.kv("FIN_SELECTORS")?;
let result = auto_update_invalid_selectors(url, code_type, &new_selectors, &kv).await?;
for (selector, is_valid) in result {
    println!("{}: {}", selector, is_valid);
}
Ok(())
